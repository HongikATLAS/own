...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        with dt[0]:
            ...
            # ROI좌표
            polygons_left = np.array([[(w//4, h//4), (0, h), (w//4, h), (w//2, h//4)]])    
            polygons_right = np.array([[(w//2, h//4), (3*w//4, h), (w, h), (3*w//4, h//4)]])

            img = im0s[0]

            # 전처리 + ROI로 자르기
            canny_image = filt(img)
            cropped_image1 = ROI(canny_image, polygons_left)          
            cropped_image2 = ROI(canny_image, polygons_right)
            cropped_image= cv2.bitwise_or(cropped_image1, cropped_image2)
            cv2.imshow('aa',cropped_image)

            # ROI 선 표시
            L_ROI_image = display_ROI(img, ROI_lines(polygons_left))
            R_ROI_image = display_ROI(img, ROI_lines(polygons_right))
            ROI_image = cv2.bitwise_or(L_ROI_image, R_ROI_image)
            #
            #if Red:
            #     # 차선 인식
            lines = cv2.HoughLinesP(cropped_image, 1, np.pi / 180, 10, minLineLength=20, maxLineGap=2)
            left, right, stop, No_Line, y_stack = average_slope_intercept(lines, w, h, y_stack)
            #
